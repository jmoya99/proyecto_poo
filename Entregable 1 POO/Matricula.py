...
class Matricula:
    ...
    def to_string(self):
        return "{0}: {1}, {2}: {3}, {4}{5}, {6}: {7}".format(Mensajes.mensa["sem"], self.get_semestre(), Mensajes.mensa["notFinal"],
                                                             self.get_notafinal(
        ), Mensajes.mensa["numGrp"], self.get_grupo().get_numero(),
    ...
    def buscar_matricula(list_matricula, id_estudiante, id_materia):
        for ma in list_matricula:
            gr = ma.get_grupo()
            if(int(ma.get_estudiante().get_identificacion()) == int(id_estudiante) and int(gr.get_materia().get_id()) == int(id_materia)):
                return ma
        return None
    ...
    def eliminar_por_grupo(lista, num_grupo, id_materia):
        ...
        for i in range(0, len(lista)):
            if(lista[i - borr].get_grupo().get_numero() == num_grupo and lista[i - borr].get_grupo().get_materia().get_id() == id_materia):
                Matricula.cancelar(
                    lista, lista[id_estudiante - borr].get_estudiante().get_identificacion(), num_grupo, id_materia)
                borr += 1
    ...
    def eliminar_por_estudiante(lista, estu):
        ...
        for i in range(0, len(lista)):
            if(lista[i - borr].get_estudiante().get_identificacion() == estu):
                ...
                Matricula.cancelar(
                    lista, estu, gru.get_numero(), gru.get_materia().get_id())
                borr += 1
